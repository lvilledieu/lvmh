  ...
  def handle_cast({:create, name}, state) do
    ...
    case lookup(state.names, name) do
      {:ok, _pid} ->
        {:noreply, state}
      :error ->
        {:ok, pid} = Serv_supervisor.start_bucket(state.buckets)
...
def handle_cast(object, intern_state) do
  ...
  def create(tableName, datas) do
    :ets.insert_new(tableName, datas)
  end

  def read(tableName,filter) do
    :ets.lookup(tableName, filter)
  end

  def update(tableName,datas) do

  end

  def delete(tableName) do
